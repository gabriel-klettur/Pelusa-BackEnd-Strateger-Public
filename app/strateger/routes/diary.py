...
async def read_diary_entry(entry_id: str, db: AsyncSession = Depends(get_db)):
...
async def read_diary_entries(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
...
async def update_diary_entry(entry_id: str, entry: DiaryEntryUpdate, db: AsyncSession = Depends(get_db)):
    db_entry = await get_diary_entry(db, entry_id)
    if db_entry is None:
        raise HTTPException(status_code=404, detail="Entry not found")
...
async def delete_diary_entry(entry_id: str, db: AsyncSession = Depends(get_db)):
    db_entry = await get_diary_entry(db, entry_id)
    if db_entry is None:
        raise HTTPException(status_code=404, detail="Entry not found")
