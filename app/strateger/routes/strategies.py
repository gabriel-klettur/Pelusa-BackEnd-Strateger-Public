...
async def read_strategies(skip: int = 0, limit: int = 10, db: AsyncSession = Depends(get_db)):
    return await get_strategies(db, skip=skip, limit=limit)

@router.post("/insert", response_model=Strategy)
async def create_strategy_endpoint(strategy: StrategyCreate, db: AsyncSession = Depends(get_db)):
    return await crud_create_strategy(db, strategy)

@router.put("/update/{strategy_id}", response_model=Strategy)
async def update_strategy_endpoint(strategy_id: int, strategy: StrategyUpdate, db: AsyncSession = Depends(get_db)):
    db_strategy = await get_strategy(db, strategy_id)
    if db_strategy is None:
        raise HTTPException(status_code=404, detail="Strategy not found")
...
async def delete_strategy_endpoint(strategy_id: int, db: AsyncSession = Depends(get_db)):
    db_strategy = await get_strategy(db, strategy_id)
    if db_strategy is None:
        raise HTTPException(status_code=404, detail="Strategy not found")
